...
\begin{enumerate}
\item $Ni=$ \emph{$\backslash E \text{ } \text{ }self \backslash in \text{ } C: P(self)$}
for a constant $C$ and $P(self)=P_p /\backslash P_n $, with $P_p$ and $P_n$ are  next-state relation and  state predicate respectively.  Then, the translation is of the form: 
\begin{lstlisting}[language=Java]
transition Ni(z) 
requires {  Trans(P_p) }
{ Trans(P_n)}
