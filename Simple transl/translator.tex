...
\Input{A TLA++ Spec based on the grammar of Section~\ref{Grammar} with \emph{TypeOk}  (Section~\ref{TypeOK} invariant and constant \emph{Proc} (Section~\ref{Proc}) as in Section.}
...
\SetKwFunction{prepa}{prepa} \SetKwFunction{algo}{algo}\SetKwFunction{proc}{proc}
  ...
  \SetKwProg{inari}{Data-Type Preparation}{}{}
         \prepa{}{
                    \For{all variables $x_i$} {
                       ...
                       \uIf(){$Type(x_i) =String $}{ Define an abstract Cubicle-type that involves all possible values of $x_i$ 
  ...
  \myalg{}{
  \tcp{Recall that Init == $ \bigwedge\limits_{i=1}^n x_i = Value_i $  }
  \For{$i=1$ \KwTo $n$}
      {   
         \uIf(){$Type(Value) \in \{Integer, Boolean\} $}{
         ...
         {
         \tcp{with $Type(Value_{i,z}) \in \{String, Boolean, String\}$}
 ...
 \SetKwBlock{transition}{Defining Transition $T_i$}{transition}{}
  ...
  \myproc{}{ 
             ...
             \nl $K^{\prime}$ is an inclusion-wise maximal  subset of $\{1,\dots, k\}$ with $Predicate_i,Predicate_j$ are not equivalent for all $1\leq i\neq j \leq  k^{\prime}$  \

             \ForEach{$i \in K^{\prime}$} 
                                    {  
...
\usebox{\codeboxu}
 ...
 \Return  Cubicle-spec with the computed \emph{unsafe (z), Init (z)}  and \emph{$T_1(z),\dots,T_{|K^{\prime}|}(z)$}.
